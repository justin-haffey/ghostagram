...
title: Azure naming convensions
description: Standards and convensions for naming and tracking Azure resources
author: justin@ghostworx.ai
---
...
|-------|-------------|---------|
| **Global** | Unique across all of Azure. This scope applies to PaaS resources with public IP endpoints, as their names serve as the initial default public DNS name. | For example, a web app named `navigator-prod-app-001.azurewebsites.net` must be globally unique. |
| **Resource group** | Unique within the resource group. Resources in the same group can't share the same name, but identical names can exist in different resource groups. | For example, a virtual network named `prod-westus-001-vnet` can exist in multiple resource groups, but only once within a single resource group. |
...
|--|--|--|--|
| **Resource type** | An [abbreviation](./resource-abbreviations.md) that represents the type of Azure resource or asset. | `rg`, `vm`, `st`, `app` | `navigator-prod-001-rg`, `web-prod-001-vm` |
| **Workload, application, or project** | Name of a workload, application, or project that the resource is a part of. | `navigator`, `emissions`, `sharepoint`, `hadoop` | `app-navigator-prod`, `sharepoint-dev-vm-001` |
| **Environment** | Production, development, quality assurance, stage, test. | `prod`, `dev`, `qa`, `stage`, `test` | `navigator-prod-app`, `dev-sqldb` |
| **Region** | The [Azure region](/azure/reliability/availability-zones-service-support) or cloud provider location where the resource is deployed. | `westus`, `eastus2`, `westeu`, `usva`, `ustx` | `shared-eastus2-001-vnet`, `hadoop-prod-westus-pip-001` |
...
- Suffix resources with the abbreviation defined in the **`Abbreviations`** tables below
- `<app|project :optional>`-`<environment :optional>`-`<suffix :required>`-`<instance ## : optional>`
- **Examples**: `theproject-kv` or `secrets-dev-kv`

### Resource groups

- Suffix resource groups with `-rg`
- Capitalize resource group names
- `RG`-`<workload,app, or project>`-`<environment>`-`<instance ##>`
- **Examples**: `RG-SAMPLEPROJ-DEV` or `RG-SAMPLEPROJECT-DEV-01`

## Abbreviations

This section provides recommended abbreviations for resources in Azure by category. The following tables have *abbreviations* mapped to *resource* and *resource provider namespace*.
