...
class GhostagramEngine {
  ...
  renderNode(node) {
    ...
    const usesFullLabelLayout = nodeUsesFullLabelLayout(node, layout);
    const header = document.createElement("header"); header.className = "ghostagram-node-header"; header.style.cssText = usesFullLabelLayout
      ? "position:absolute;inset:6px;display:block;box-sizing:border-box;z-index:2;"
      : `position:absolute;left:6px;right:6px;top:0;height:${NODE_PROPERTY_TOP}px;display:flex;align-items:center;gap:5px;box-sizing:border-box;z-index:2;`;
    label.style.cssText = nodeLabelStyle(usesFullLabelLayout);
...
function nodeUsesFullLabelLayout(node, layout) { return !layout.progressive && (node.properties?.length ?? 0) === 0 && layout.rows.length === 0; }
function nodeLabelStyle(wrap = false) {
  return wrap
    ? "display:block;min-width:0;height:100%;box-sizing:border-box;overflow:hidden;overflow-wrap:anywhere;padding-right:22px;white-space:normal;line-height:1.2;"
    : "display:block;flex:1 1 0;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;line-height:1.2;";
}
