...
public sealed class SvgDiagramExporter : IDiagramExporter
{
    ...
    public DiagramExportArtifact Export(DiagramSnapshot snapshot)
    {
        ...
        foreach (var edge in edges.OrderBy(item => item.Id, StringComparer.Ordinal))
        {
            ...
            svg.Append("<g data-edge-id=\"").Append(Escape(edge.Id)).Append("\"><path d=\"M ").Append(N(source.X)).Append(' ')
                .Append(N(source.Y)).Append(" L ").Append(N(target.X)).Append(' ').Append(N(target.Y))
        ...
        foreach (var node in nodes.Values.OrderBy(item => item.Id, StringComparer.Ordinal))
        {
            svg.Append("<g data-node-id=\"").Append(Escape(node.Id)).Append("\"><rect x=\"").Append(N(node.Box.X))
                ...
                .Append(Text(node.Box.X + 12, node.Box.Y + node.Box.Height / 2 + 5, node.Box.Label, 15, "#0f172a")).Append("</g>");
        }
        ...
        out Point point)
    {
        ...
        point = port.Anchor.ToLowerInvariant() switch
        {
            ...
            _ => new(box.X + box.Width / 2, box.Y + box.Height / 2)
        };
        return true;
    }
    ...
    private sealed record Node(string Id, Box Box)
    {
        public static Node Parse(JsonElement item)
        {
            var box = Box.Parse(item);
            return new(box.Id, box);
        }
    }
    private sealed record Port(string Id, string NodeId, string Anchor)
    {
        public static Port Parse(JsonElement item) => new(S(item, "id"), S(item, "nodeId"), S(item, "anchor", "center"));
    }
    private sealed record Edge(string Id, string SourcePortId, string TargetPortId, string Label)
    {
        public static Edge Parse(JsonElement item) => new(S(item, "id"), S(item, "sourcePortId"), S(item, "targetPortId"), S(item, "label"));
    }
}
