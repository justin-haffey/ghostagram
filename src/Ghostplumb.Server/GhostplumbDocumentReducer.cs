...
/// <summary>
/// The DOM-free authoritative subset of the Ghostplumb operation protocol.
/// Rendering-only viewport fit/center requests are intentionally not persisted.
/// </summary>
public static class GhostplumbDocumentReducer
{
    private static readonly HashSet<string> Collections = ["nodes", "ports", "edges", "groups", "edgeTypes"];

    public static JsonElement Apply(JsonElement model, IReadOnlyList<GhostplumbOperation> operations)
    {
    ...
    private static void ApplyOne(JsonObject root, GhostplumbOperation operation)
    {
    ...
    private static JsonObject ObjectValue(GhostplumbOperation operation)
    {
    ...
    private static string Id(GhostplumbOperation operation, JsonObject value) => operation.Id ?? IdOf(value);
